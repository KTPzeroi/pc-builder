...
async def run_test():
    ...
    try:
        ...
        # -> Click the LOGIN button to open the sign-in form (element index 441).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/nav/header/ul[2]/li/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the username and password fields with the provided invalid credentials, submit the login form, then check for a visible invalid-credentials error message.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=อีเมลหรือรหัสผ่านไม่ถูกต้อง').first).to_be_visible(timeout=3000)
