...
async def run_test():
    ...
    try:
        ...
        # -> Fill the Username/Email and Password fields (clearing them first) with admin / 1234 and submit the form.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Total Users').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Latest Reports').first).to_be_visible(timeout=3000)
