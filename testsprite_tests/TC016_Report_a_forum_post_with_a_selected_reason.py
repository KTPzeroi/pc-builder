...
async def run_test():
    ...
    try:
        ...
        # -> Fill the Username/Email field with 'admin' (clear first), fill Password with '1234' (clear first), then click the Log In button.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the Log In button to ensure submission, wait for the page to respond, then navigate to /forum.
        frame = context.pages[-1]
        # Click element
        ...
        # -> Open the first forum post titled 'Test Forum Post' by clicking it.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/main/div/div[2]/div/div/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Open the post options menu by clicking the '•••' button so the report option is revealed.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Report Submitted').first).to_be_visible(timeout=3000)
